...
loanRouter.get('/', ensureAuthenticated, restrictToRoles('admin', 'banker'), async (request, response) => {
    try {
        const [allLoans] = await database.execute('SELECT l.*, u.name as customer_name, u.email as customer_email FROM loans l JOIN users u ON l.customer_id = u.id');
...
loanRouter.get('/customer/:customerId', ensureAuthenticated, restrictToRoles('customer', 'banker', 'admin'), async (request, response) => {
    ...
    try {
        const [customerLoans] = await database.execute('SELECT * FROM loans WHERE customer_id = ?', [customerId]);
...
loanRouter.put('/:id/status', ensureAuthenticated, restrictToRoles('banker'), async (request, response) => {
    ...
    try {
        const [updateResult] = await database.execute('UPDATE loans SET status = ? WHERE id = ?', [status, loanId]);
